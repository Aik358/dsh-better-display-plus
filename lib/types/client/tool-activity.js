...
function childIdentity(block) {
    if (!block || !('kind' in block))
        return { name: block?.name, raw: callArgs(block) };
...
export function activityPhase(entry, turnClosed = false) {
    ...
    if (entry.block.isError || facts.signal || (facts.exitCode !== undefined && facts.exitCode !== 0)
        || (Array.isArray(entry.block.subCalls) && entry.block.subCalls.some(block => activityPhase({ block }, turnClosed) === 'failed')))
        return 'failed';
    if (facts.exitCode === 0)
        return 'succeeded';
    return 'returned';
