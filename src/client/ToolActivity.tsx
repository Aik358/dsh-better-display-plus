...
import { IconOther, IconRead, IconSearch, IconSkill, IconTerminal, IconWeb, IconWrite } from './icons.js';
...
const ICONS = { write: IconWrite, read: IconRead, terminal: IconTerminal, search: IconSearch, web: IconWeb, code: IconTerminal, other: IconOther } satisfies Record<ToolCategory, unknown>;
...
export const ToolActivity = memo(function ToolActivityView({ entry, motion, turnClosed, onRead, depth = 0, ...render }: BlockRenderProps & {
  ...
  const Icon = model.name === 'skill' ? IconSkill : ICONS[model.category];
  const block = entry.block;
