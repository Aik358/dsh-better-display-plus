...
function Summary({ item, open, onChange, motion }) {
    ...
    return _jsx("div", { "data-reader-live-fold": true, "data-expanded": open, "data-reader-live-fold-summary": item.summary, "data-reader-fold-named": item.named ? 'on' : undefined, children: _jsxs("div", { className: css.summaryRow, children: [_jsx(Disclosure, { open: open, onChange: onChange, buttonRef: button, ariaLabel: "\u6B64\u524D\u6B65\u9AA4", label: _jsx(FoldSummaryText, { summary: item.summary, motion: motion }) }), _jsx(DiffStat, { steps: item.steps, label: item.summary })] }) });
}
...
export function ChoreographedFlow({ frame, motion, enabled, urgent, open, onOpenChange, processOpen, renderStep, id }) {
    ...
    const isOpen = (key) => processOpen && !!open[key];
    ...
    return _jsx(FlowSnapshot.Provider, { value: source.snapshot, children: _jsx(StreamMotionContext.Provider, { value: { ...stream, paused: state.phase !== 'idle', resumed: state.phase === 'reveal' }, children: _jsx("div", { id: id, ref: root, className: css.choreographedFlow, "data-reader-flow": true, "data-reader-transition": state.phase, "data-ud-motion": "fold-choreography", children: rows.map(row => {
                    if (row.kind === 'summary')
                        return _jsx(FlowCell, { rowKey: row.key, hidden: false, instant: true, motion: motion, summary: true, children: _jsx(Summary, { item: row.item, open: isOpen(row.key), onChange: value => onOpenChange(row.key, value), motion: motion && state.phase !== 'collapse' }) }, row.key);
