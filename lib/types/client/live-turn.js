 ...
 */
export function settledFoldItems(steps, boundary, keepProse, keepToolSemantics = false) {
    ...
    if ((!keepProse && !keepToolSemantics) || boundary.status === 'open')
        return null;
...
function clipTarget(value, max = 32) {
    const firstLine = value.split(/\r?\n/u)[0].trim();
    return firstLine.length <= max ? firstLine : `${firstLine.slice(0, max)}…`;
}
export function foldToolSemantics(steps, limit = 3) {
    const named = [];
    for (const step of steps) {
        if (step.kind !== 'tool')
            continue;
 ...
 */
export function splitChainKeepingBody(chain, sealed = false) {
    ...
    const flushRun = (finished) => {
        ...
        if (finished && worthFolding)
            segments.push({ fold: run, open: [] });
        else
            segments.push({ fold: null, open: run });
        run = [];
    };
...
export function presentLiveTurn(steps, boundary, autoFold = true, keepProse = false, keepToolSemantics = false, sealed = false) {
    const live = autoFold && liveFoldEnabled(boundary);
    const items = [];
    let chain = [];
    const pushSegment = (segment, fallbackKey) => {
        ...
        for (const step of segment.open)
    ...
    const flush = () => {
        ...
        if (keepProse || keepToolSemantics) {
            ...
            for (const segment of splitChainKeepingBody(chain, sealed))
                pushSegment(segment, chain[0].key);
            chain = [];
            return;
        }
