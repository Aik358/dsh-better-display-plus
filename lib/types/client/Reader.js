...
function pendingInteractionOf(snapshot) {
    const value = snapshot?.pendingInteraction;
...
const ProcessNode = memo(function ProcessNode({ useChat, t, nodeKey, open, motion, onRead, returnFocusTo }) {
    ...
    if (isNode(node, 'context'))
        content = _jsx(ContextInjectionRow, { ...node.data, t: translateOf(t) });
...
function GroupStatus({ group, useChat, useSession, motion }) {
...
const TurnGroup = memo(function TurnGroup({ group, motion, autoFold, keepProse, keepToolSemantics, foldLevel, pinnedKeys, selectedProcessKeys, isAwaitingModel = false, ...props }) {
    ...
    const liveItems = useMemo(() => presentLiveTurn(steps, boundary, autoFold, keepBody, keepToolSemantics), [steps, boundary, autoFold, keepBody, keepToolSemantics]);
    ...
    const settled = useMemo(() => settledFoldItems(steps, boundary, autoFold && keepBody, keepToolSemantics), [steps, boundary, autoFold, keepBody, keepToolSemantics]);
    ...
    const liveRates = useMemo(() => {
        ...
        if (ttft !== undefined || rate !== undefined)
            return { ttftMs: ttft, tokensPerSecond: rate };
        let first;
        let last;
        let outputChars = 0;
        for (const step of turn?.steps ?? []) {
            ...
            if (typeof timing.completedTime === 'number')
                last = Math.max(last ?? timing.completedTime, timing.completedTime);
        ...
        if (!hasWindow)
            return { ttftMs: undefined, tokensPerSecond: undefined };
        return {
            ttftMs: first,
    ...
    return _jsxs("section", { className: css.turn, "data-reader-turn": group.turn ?? 'unresolved', "data-reader-turn-state": boundary.status, "data-reader-turn-result": boundary.reason ?? undefined, children: [startsWithUser && _jsx(BlockBoundary, { children: _jsx(MainNode, { ...shared, useChat: props.useChat, boundary: boundary, nodeKey: group.keys[0] }) }), hasProcess && !isAwaitingModel && _jsx(StickyLane, { kind: "status", className: css.turnProcessSticky, children: _jsx(Disclosure, { open: expanded, onChange: setExpanded, controls: flowId, buttonRef: processButton, label: _jsx(GroupStatus, { group: group, useChat: props.useChat, useSession: props.useSession, motion: motion }) }) }), boundary.status === 'closed' && hasProcess && autoFold && !settled && _jsx(ClosedProcessSummary, { open: expanded, onChange: setExpanded, controls: flowId, steps: steps.filter(step => {
                ...
                }) }), _jsx(ChoreographedFlow, { id: flowId, frame: presentation, motion: motion, enabled: autoFold && (boundary.status === 'open' || !!settled) && !holdingSelection, urgent: hasTurnError || interaction !== undefined || !sessionRunning, open: foldOpenByKey, processOpen: expanded, onOpenChange: (key, value) => { setFoldOpenByKey(current => ({ ...current, [key]: value })); }, renderStep: renderStep }), !hasProcess && boundary.status === 'open' && !isAwaitingModel && _jsx("div", { className: css.disclosure, "data-reader-status-only": true, children: _jsx(GroupStatus, { group: group, useChat: props.useChat, useSession: props.useSession, motion: motion }) }), showDeliverablesRow(boundary.status, deliverables) && _jsx(DeliverablesRow, { deliverables: deliverables, openFile: props.openFile, revealFile: props.revealFile, openMode: openMode }), showTerminalNotice && _jsx("div", { className: css.notice, "data-reader-terminal": true, children: terminal })] });
});
export function Reader(props) {
    ...
    return _jsx(StreamMotionContext.Provider, { value: streamMotion, children: _jsxs("div", { ref: root, className: css.root, "data-dsh-better-display": "0.2.1", "data-reader-wait-clock-version": "input-v1", "data-reader-wait-start": waitAnchor.time ?? undefined, "data-motion": motion ? 'on' : 'off', "data-reader-glass": frostedGlass || undefined, "data-reader-auto-fold": autoFold ? 'on' : 'off', children: [_jsx(TimelineRail, { items: timelineItems, activeTurn: activeTurn, busyTurn: busyTurn, onNavigate: onNavigateTurn }), _jsxs("div", { className: css.column, "data-chat-flow": "", children: [_jsx(StickyLane, { kind: "toolbar", className: css.toolbar, children: _jsx("button", { type: "button", className: css.textButton, "aria-pressed": autoFold, onClick: () => {
                        ...
                        }), isAwaitingModel && _jsx(WaitingStatus, { anchor: waitAnchor, label: translateOf(props.t)('chat.deepDiving') }), pending !== undefined && _jsxs("div", { className: css.attention, role: "alert", "data-reader-attention": true, children: [_jsx("strong", { children: pending.kind === 'question' ? '需要你回答一个问题' : '需要你的确认' }), _jsx("span", { children: "\u8BF7\u5728\u4E0B\u65B9\u539F\u751F\u64CD\u4F5C\u533A\u5904\u7406\u3002\u6B64\u63D0\u793A\u4E0D\u4F1A\u6536\u8FDB\u6267\u884C\u8FC7\u7A0B\u3002" })] }), scroll.detached && _jsx("div", { className: css.jumpDock, children: _jsx("button", { type: "button", className: css.jump, "aria-label": "\u56DE\u5230\u5E95\u90E8", title: "\u56DE\u5230\u5E95\u90E8", onClick: scroll.jump, children: _jsx("svg", { viewBox: "0 0 14 14", width: "14", height: "14", "aria-hidden": "true", children: _jsx("path", { d: "M3 5.5 7 9.5 11 5.5", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round" }) }) }) })] })] }) });
}
